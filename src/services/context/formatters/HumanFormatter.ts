...
const messages: Record<ContextLang, Record<string, string>> = {
  ...
  zh: {
    ...
    columnKey: '说明',
    readLabel: '读取成本',
    readDescription: '现在理解这条观察记录，大约需要读入多少 Token',
    workLabel: '生成成本',
    workDescription: '当初产出这条记录大约花了多少 Token（研究、实现、决策）',
    contextIndex: '记忆索引：这里只列出标题、类型、文件和 Token，一般已经够你快速了解之前做过什么。',
    implementationHelp: '如果你还想看实现细节、设计原因或排查过程：',
    fetchById: '  - 按 ID 查看：对当前索引里能看到的记录使用 get_observations([IDs])',
    searchHistory: '  - 搜索历史：使用 mem-search skill 查找以前的决策、Bug 处理和更深入的研究',
    trustIndex: '  - 想快速了解来龙去脉时，先看这个索引，通常比重新翻代码更省',
    contextEconomics: 'Token 开销',
    loadingLine: '  当前载入：{count} 条观察记录（预计读取 {tokens} Token）',
    workInvestmentLine: '  这些记录当初共花了 {tokens} Token 用于研究、实现和决策',
